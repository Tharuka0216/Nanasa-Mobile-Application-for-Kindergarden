...
const ParentLetterScreen = () => {
  ...
  const loadUserInfo = async () => {
    try {
      ...
      if (asyncUserName !== null) {
        loadAllLetterRecords(asyncUserName);
      } else {
      }
    } catch (error) {
    }
  ...
  const loadAllLetterRecords = (username) => {
    try {
      fetch(GET_ALL_RECORD, {
        ...
        .then((data) => {
          const lastValue = data.letters.map((item) => item.marks);

          const newChartData = {
            labels: data.letters.map((dataPoint) => {
            ...
            datasets: [
              {
                data: data.letters.map((dataPoint) => dataPoint.marks),
              },
            ],
          };
          if (lastValue.length > 0) {
            const lastValues = lastValue[lastValue.length - 1];
            setLastWritten(lastValues);
          } else {
...
export default ParentLetterScreen;
